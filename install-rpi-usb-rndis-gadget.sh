...
patch_raspi_config_networkmanager_wifi() {
  [[ -f /usr/bin/raspi-config ]] || return 0
...
EOF
  ...
  log "Patching raspi-config NetworkManager Wi-Fi handling"
  patch_raspi_config_networkmanager_wifi
